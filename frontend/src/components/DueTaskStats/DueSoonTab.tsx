...
export const DueSoonTab = ({ tasks }: { tasks: OverdueTask[] }) => {
        ...
        <Tabs.Panel value="due_soon">
            ...
            <div className="time-sensitive-tasks-container">
                <ul className="time-sensitive-tasks">
                    {tasks.length === 0 ? <div className="sensitive-tasks-empty">You have no tasks due in the next 7 days.</div> : tasks.map(task => (
                        <TimeSensitiveTask
                            key={task.id}
                            task={task}
