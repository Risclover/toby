...
def upgrade():
    ...
    sa.Column('created_at', sa.DateTime(), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True),
    sa.Column('updated_at', sa.DateTime(), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True),
    sa.CheckConstraint('(user_id IS NOT NULL) <> (household_id IS NOT NULL)', name='ck_tasklists_exactly_one_owner'),
    sa.ForeignKeyConstraint(['household_id'], ['households.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('tasklists', schema=None) as batch_op:
        ...
        batch_op.create_index('ix_tasklists_user_id', ['user_id'], unique=False)
    ...
    sa.PrimaryKeyConstraint('tasklist_id', 'user_id')
    )
    ...
    sa.ForeignKeyConstraint(['list_id'], ['tasklists.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
...
def downgrade():
    ...
    with op.batch_alter_table('tasklists', schema=None) as batch_op:
        batch_op.drop_index('ix_tasklists_user_id')
