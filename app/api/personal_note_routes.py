...
@personal_note_routes.route("/<string:id>", methods=["GET"])
@login_required
def get_note(id):
...
def create_note():
    ...
    if len(body) > 10000:
        return jsonify({"error": "Body must be 10,000 characters or fewer"}), 400
...
@personal_note_routes.route("/<string:id>", methods=["DELETE"])
@login_required
def delete_note(id):
    note = PersonalNote.query.get(id)
